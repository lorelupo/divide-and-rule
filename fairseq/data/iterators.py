    ...
    def next_epoch_itr(self, shuffle=True, fix_batches_to_gpus=False):
        ...
        if self._next_epoch_itr is not None:
            ...
            self._cur_epoch_itr = self._get_iterator_for_epoch(
                self.epoch, shuffle=shuffle, fix_batches_to_gpus=fix_batches_to_gpus,
            )
...
class EpochSplitAndBatchIterator(EpochBatchIterator):
    ...
    def __init__(
        ...
        # create a batch sampler this is useful only for
        # the first_batch property defined in EpochBatchIterator
        indices = self.dataset.ordered_indices(self.seed + epoch)
        ...
        super().__init__(
            dataset, collate_fn, batch_sampler, seed,
            num_shards, shard_id, num_workers, epoch, buffer_size
        )
    
    def _get_iterator_for_epoch(self, epoch, **kwargs):

        offset = kwargs.get('offset', 0)

        # get sentence ids (again)
        indices = self.dataset.ordered_indices(self.seed + epoch)

...
def collate(samples,
    ...
    def check_alignment(alignment, src_len, tgt_len):
        if alignment is None or len(alignment) == 0:
            return False
        if alignment[:, 0].max().item() >= src_len - 1 or alignment[:, 1].max(
        ).item() >= tgt_len - 1:
            logger.warning(
    ...
    # sort_order = torch.arange(len(id))
    ...
    if doc_heads is not None:
        batch['net_input']['doc_heads'] = doc_heads
    ...
    if samples[0].get('alignment', None) is not None:
        ...
        offsets[:, 1] += (torch.arange(len(sort_order), dtype=torch.long) *
                          tgt_sz)
        if left_pad_source:
...
class DocLanguagePairDataset(LanguagePairDataset):
    def __init__(
        ...
        self.exclude_indices = None  # indices of sentences that we never want to employ
    ...
    def collater(self, samples):
        ...
        batch = collate(samples,
                        ...
                        left_pad_target=self.left_pad_target,
                        input_feeding=self.input_feeding,
                        id_to_net_input=True,
                        doc_heads=self.doc_heads)

        return batch

    def ordered_indices(self, seed=0, exclude_indices=None):
        """
        ...
        """
        if not self.shuffle_sents:

            def shuffle_docs_indices(indices, seed):
                with data_utils.numpy_seed(seed):
                    np.random.shuffle(indices)
                return indices

            # batch corpus sentences in the original order
            if self.shuffle_docs:
                indices = np.split(np.arange(len(self)), self.doc_heads)
                indices = np.concatenate(shuffle_docs_indices(indices, seed))
            else:
                indices = np.arange(len(self))
        else:
            logger.info(
                'shuffling dataset sentences, will batch by increasing sentence length.'
                )
            # randomize sentence order: useful for ablation studies
            indices = np.random.permutation(len(self))
            if self.tgt_sizes is not None:
                # sort indices by target sentence sizes (for performance)
                indices = indices[np.argsort(self.tgt_sizes[indices],
                                             kind='mergesort')]
            # sort indices by source sentence sizes
            indices = indices[np.argsort(self.src_sizes[indices],
                                         kind='mergesort')]

        if exclude_indices is None:
            return indices
        else:
            return np.array([i for i in indices if not i in exclude_indices])

...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            # modify the loaded state if it does not match the current model
            state = self.task.match_pretrained_to_arch(state, self.get_model())
    ...
    def get_train_iterator(
        ...
        """Return an Epoch(SplitAnd)BatchIterator over the training set for a given epoch."""
        if load_dataset:
